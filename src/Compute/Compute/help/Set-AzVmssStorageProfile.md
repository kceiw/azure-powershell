...
PS C:> Set-AzVmssStorageProfile -ImageReferenceOffer $ImgRef.Offer -ImageReferencePublisher {ImageReferencePublisher} -ImageReferenceSku $ImgRef.Skus -ImageReferenceVersion $ImgRef.Version -OsDiskCaching None -OsDiskCreateOption FromImage -VirtualMachineScaleSet ContosoVMSS
```

The Set-AzVmssStorageProfile cmdlet sets the storage profile properties for the Virtual Machine Scale Set (VMSS). (autogenerated)
