...
PS C:> Get-AzHDInsightJob -ClusterName $clusterName -HttpCredential $clusterCreds -JobId $jobId
```

The Get-AzHDInsightJob cmdlet gets recent jobs for a specified Azure HDInsight cluster in reverse chronological order, with the most recent job at the top of the list (autogenerated)
