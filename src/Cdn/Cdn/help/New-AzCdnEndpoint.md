...
PS C:> New-AzCdnEndpoint -EndpointName $endpointName -Location westus -OriginHostHeader {OriginHostHeader} -OriginHostName {OriginHostName} -OriginName {OriginName} -OriginPath {OriginPath} -ProfileName $profileName -ResourceGroupName MyResourceGroup
```

The New-AzCdnEndpoint cmdlet creates an Azure Content Delivery Network (CDN) endpoint. (autogenerated)
