...
PS C:> Save-AzApiManagementTenantGitConfiguration -Branch master -Context $apimContext -Force  -PassThru 
```

The Save-AzApiManagementTenantGitConfiguration cmdlet saves the changes by creating a commit that contains the current configuration snapshot to a branch in the repository. (autogenerated)
