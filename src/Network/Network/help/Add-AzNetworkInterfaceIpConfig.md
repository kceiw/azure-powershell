...
PS C:> Add-AzNetworkInterfaceIpConfig -LoadBalancerBackendAddressPool {LoadBalancerBackendAddressPool} -Name MyNewIpConfig -PrivateIpAddressVersion IPv4 -Subnet $vnet.Subnets[0]
```

The Add-AzNetworkInterfaceIpConfig cmdlet adds a network interface IP configuration to an Azure network interface. (autogenerated)
