...
PS C:> New-AzLoadBalancer -BackendAddressPool $BackendAddressPool -Force  -FrontendIpConfiguration $Frontend -InboundNatPool {InboundNatPool} -LoadBalancingRule $lbrule -Location $LOC -Name $LBName -Probe $probe -ResourceGroupName $RGName
```

The New-AzLoadBalancer cmdlet creates an Azure load balancer. (autogenerated)
