...
### Example 1: 1: Create an IP configuration with a public IP address for a network interface
```powershell
...
### Example 2: 2: Create an IP configuration with a private IP address
```powershell
$vnet = Get-AzVirtualNetwork -Name myvnet -ResourceGroupName myrg
$Subnet = Get-AzVirtualNetworkSubnetConfig -Name mysubnet -VirtualNetwork $vnet
...
The New-AzNetworkInterfaceIpConfig cmdlet creates an Azure network interface IP configuration for a network interface. (autogenerated)
