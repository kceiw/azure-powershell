...
### Example 1: 1: Changing the IP address of an IP configuration
```powershell
...
### Example 2: 2: Associating an IP configuration with an application security group
```powershell
$vnet = Get-AzVirtualNetwork -Name myvnet -ResourceGroupName myrg
$subnet = Get-AzVirtualNetworkSubnetConfig -Name mysubnet -VirtualNetwork $vnet
...
PS C:> Set-AzNetworkInterfaceIpConfig -ApplicationSecurityGroup $asg -Name $nic.IpConfigurations[0].Name -NetworkInterface $sourceNic -Subnet $subnet
```

The Set-AzNetworkInterfaceIpConfig cmdlet updates an IP configuration for a network interface. (autogenerated)
